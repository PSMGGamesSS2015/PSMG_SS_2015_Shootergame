...
//		    float spec = pow (nDotfloat, _Shininess * 128.0) * _Gloss;
...
//		    float distrTerm = pow (nDotfloat, _Shininess * 128.0) * _Gloss;
//		    
//		    //Cook-Torrance geometry term
//		    float geometryTerm = (2.0 * ( nDotfloat/dot(normViewDir, halfView) ) *  min(nDotView, nDotLight) );
...
//		    #endif
		    ...
		    half specularComponent = spec * _LightColor0.r;
	...
	//				    glossValue = min(0.8, glossValue);
	    ...
	    //float specIntensity = tex2D( _WaterMap, (uv_MainTex + nDotfloat * 100.0) * .01 ).b * tex2D( _WaterMap, (uv_MainTex + nDotfloat * 100.0) * .1 ).b;// *
	   	//					 tex2D( _WaterMap, (uv_MainTex + nDotfloat) *.3 ).b;
...
//	    }
		...
		fixed3 diffuseComponent = (nDotLight * .2 + .8) * _LightColor0.rgb;// * silhouetteness;
...
//						#endif
	    ...
	    //Reflectivity
			...
			//refl.xz *= -1.0;
			//float3 refl = reflect( -normViewDir, float3(0.0, 1.0, 0.0) );
			//refl.xz *= 3.0;
			//refl = normalize(refl);
		...
		outColor.rgb = CombineEffectsWithLighting(
									...
									//float3(tex2Dlod(_HeightGlossDUDVMap, float4(i.uv_MainTex, 0, 0) ).b, 0.0, tex2Dlod(_HeightGlossDUDVMap, float4(i.uv_MainTex, 0, 0)).a),
											...
											#else
												#if !defined(CALCULATE_NORMALS_ON) && defined(REFRACTIONS_ON)
												,half3(i.anisoDir.xy + _dudvValue.xy, i.anisoDir.z)
												#else
												,i.anisoDir
												#endif
											#endif
